...
- `logical_classes.py`: Conotains classes for each type of logical compnent, e.g., `Fact`, `Rule`, etc.
...
'Statement({!r}, {!r})'.format(self.predicate, self.terms)

'Term({!r})'.format(self.term)

'Variable({!r})'.format(self.element)

'Constant({!r})'.format(self.element)


'Binding({!r}, {!r})'.format(self.variable, self.constant)


'Bindings({!r}, {!r})'.format(self.bindings_dict, self.bindings)


'ListOfBindings({!r})'.format(self.list_of_bindings)