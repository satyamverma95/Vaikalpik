...
class KBTest(unittest.TestCase):
    ...
    def setUp(self):
        ...
        #print("What data we have received", data)
        for item in data:
            if isinstance(item, Fact) or isinstance(item, Rule):
                self.KB.kb_assert(item)
        #print("Facts in Knowledge Base", self.KB.facts)
        #print("Rule in Knowledge Base", self.KB.rules)

    def test1(self):
        ask1 = read.parse_input("fact: (motherof ada ?X)")
        #ask1 = read.parse_input("fact: (aunt eva ?X)")
        #print("Whata are we geting in ask", ask1)
    ...
    def test3(self):
        ...
        if unittest.main.verbosity > 1:
            print(' Retracting', r1)
        print("Facts in Knowledge Base Before ", self.KB.facts)
        self.KB.kb_retract(r1)
        #print("Facts in Knowledge Base After Retract ", self.KB.facts)
        ask1 = read.parse_input("fact: (grandmotherof ada ?X)")
        if unittest.main.verbosity > 1:
            print(' Asking if', ask1)
        answer = self.KB.kb_ask(ask1)
        #print("Anwer Received ", answer)
        self.assertEqual(str(answer[0]), "?X : felix")
    ...
    '''
    def test50(self):
        r1 = read.parse_input("rule: ((manages ?x ?y)) -> (supervises ?x ?y)")
        if unittest.main.verbosity > 1:
            print(' Retracting', r1)
        self.KB.kb_retract(r1)
        ask1 = read.parse_input("fact: (manages ronak ?X)")
        ask2 = read.parse_input("fact: (substitute deepika ?X)")
        ask3 = read.parse_input("fact: (supermanages ?X ramesh)")
