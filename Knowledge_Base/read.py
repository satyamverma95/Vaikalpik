...
def read_tokenize(file):
    ...
    for line in file:
        if line[0:5] in ("fact:", "rule:"):
            ...
            #print("Putting Output", line)
            current = line.rstrip()
            #print("Storing Line", current)
        else:
...
def parse_input(e, supported_by=[]):

    #print("Input Received in Parse Input",e)
    ...
    if len(e) == 0:
        ...
        e = e[5:].replace(")", "").replace("(", "").rstrip().strip().split()   #motherof ada bing, fact:(motherof ada bing)
        #return (FACT, e)
        #print ("What is in Fact", Fact(e))
        return Fact(e, supported_by)
    elif e[0:5] == "rule:":
        #e[0] = motherof ?x ?y
        #e[1] = (parentof ?x ?y
        e = e[5:].split("->")
