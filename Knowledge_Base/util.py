...
#from logical_classes import Fact
...
def match(state1, state2, bindings=None):
    ...
    #print( "States Received state1 {}, state2 {}".format(state1, state2) )
...
def match_recursive(terms1, terms2, bindings):  # recursive...
    ...
    #print("States Received Match Recursive -- terms1 {}, terms2 {}".format(terms1, terms2))
    #print("States Received Match Recursive --len(term1):{}, len(term2):{}".format(len(terms1), len(terms2)))
    # print("PRINTING TERMS", terms1[0], terms2[0])
    if len(terms1) == 0:
        return bindings
    if is_var(terms1[0]):
